...
class ActiveRequest:
    ...
    Хранит информацию о необходимых данных (needs) и Future для асинхронного ожидания.
    """
...
class AudioStream:
    ...
    Собирает бинарные чанки аудиопотока.
    """
...
class YandexAliceClient:
    ...
    Поддерживает отправку текстовых запросов (с возможностью получения TTS-ответа)
    и прямых запросов на синтез речи (TTS).
    """
    ...
    def _get_application_payload(self) -> Dict[str, Any]:
        return {
            "app_id": "aliced_python_dialog_tts",  # Изменено для отражения специфики клиента
            "app_version": "1.0.0",
            "os_version": "CLI",
            "platform": "python_async_client",  # Консистентность с другими асинхронными клиентами
            "uuid": str(uuid.uuid4()),
            "lang": "ru-RU",
    ...
    async def _on_message(self, raw_data: Any) -> None:
        ...
        if isinstance(raw_data, str):
            try:
                ...
                print(f"Не удалось декодировать JSON из строки: {raw_data[:200]}...") # Показываем часть данных
                return
    ...
    async def _on_binary_audio(self, data: bytes) -> None:
        if len(data) < 4:
            # print("Получен слишком короткий бинарный пакет, не могу извлечь streamId.") # Менее шумный лог
            return
    ...
    async def _on_directive(self, directive: Dict[str, Any]) -> None:
        ...
        if directive_name == "Speak":
            ...
            if stream_id is not None:
                ...
                # Если 'audio' ожидается, 'audio_stream_ended' будет его сигналом завершения
                if "audio" in request_obj.needs: # Если мы ждем аудио для этого запроса
                    request_obj.needs.add("audio_stream_ended") # Добавляем ожидание конца стрима
            else:
    ...
    async def _wait_for_response(self, message_id: str, needs: Set[str]) -> Dict[str, Any]:
        ...
        def _cleanup_request(fut: asyncio.Future):
            ...
            streams_to_remove = [sid for sid, stream in list(self._active_streams.items()) if
        ...
        except asyncio.TimeoutError: # Явно ловим asyncio.TimeoutError
            err_msg = f"Таймаут ожидания ответа на запрос {message_id} (ожидались: {needs})"
            # Создаем и возбуждаем YandexAliceClientError, чтобы он был пойман в main_interactive_loop
            timeout_alice_error = YandexAliceClientError(err_msg)
            if not request_obj.future.done():
                request_obj.reject(timeout_alice_error) # Устанавливаем ошибку и на Future
            raise timeout_alice_error # Возбуждаем YandexAliceClientError
    ...
    async def send_text(self, text: str, is_tts_response: bool = False) -> Dict[str, Any]:
        ...
        except YandexAliceClientError as e:  # Перехватываем YandexAliceClientError (включая таймауты)
            error_message_from_exception = str(e)
            final_error_to_report: str

            # Если ошибка содержит "Таймаут" и OAuth токен не установлен,
            # показываем пользовательское сообщение.
            if "Таймаут" in error_message_from_exception and not self.oauth_token:
                final_error_to_report = "Не корректный запрос, возможно вы не ввели токен"
            else:
                final_error_to_report = error_message_from_exception  # Иначе показываем исходную ошибку

            return {
                "response_payload": {"error": final_error_to_report},
                "audio": None,
                "full_directives": []
            }

        # Дальнейшая обработка, если _wait_for_response НЕ вызвал исключение
        final_response_payload = {}
        vins_response_found = False
        all_directives = response_data.get("directives", [])

        if isinstance(all_directives, list):
            for directive in all_directives:
                if directive.get("header", {}).get("name") == "VinsResponse":
                    vins_response_found = True
                    payload_response = directive.get("payload", {}).get("response", {})

                    is_specific_empty_alice_payload = (
                            payload_response.get('cards') == [] and
                            payload_response.get('card') is None and
                            payload_response.get('directives') == [] and
                            not payload_response.get('text') and
                            not payload_response.get('tts')
                    )

                    if is_specific_empty_alice_payload and not self.oauth_token:
                        final_response_payload = {"error": "Не корректный запрос, возможно вы не ввели токен"}
                    elif not payload_response and not self.oauth_token:
                        final_response_payload = {
                            "error": "Не корректный запрос, возможно вы не ввели токен (ответ пуст)"}
                    else:
                        final_response_payload = payload_response
                    break

        if not vins_response_found and not final_response_payload:  # final_response_payload проверяется, чтобы не перезаписать ошибку из блока выше
            if not all_directives and not self.oauth_token:
                final_response_payload = {"error": "Не корректный запрос, возможно вы не ввели токен"}
            else:
        ...
        return {
            ...
            "full_directives": all_directives
        }
...
# ==============================================================================
# Основная логика интерактивного цикла
# ==============================================================================
async def main_interactive_loop():
    print("Чтобы остановить программу, введите 'q', 'quit' или 'exit'.")
    ...
    try:
        ...
        while True:
            ...
            print("\n[Алиса] < Отправляю запрос...")
            try:
                # Запрашиваем и текстовый ответ, и аудио (TTS ответа Алисы)
                result = await client.send_text(user_text, is_tts_response=True)

                print("\n--- Ответ Алисы ---")

                # Пытаемся извлечь текстовый ответ из разных возможных полей
                text_answer = result.get("response_payload", {}).get("card", {}).get("text")
                if not text_answer:
                    text_answer = result.get("response_payload", {}).get("text")

                if text_answer:
                    print(f"[Алиса] < Текст: {text_answer}")
                else:
                    # Если текстового ответа нет, но есть ошибка в payload
                    error_in_payload = result.get("response_payload", {}).get("error")
                    if error_in_payload:
                        print(f"[Алиса] < Ошибка в ответе: {error_in_payload}")
                    else:
                        print("[Алиса] < Не удалось извлечь прямой текстовый ответ. Полная структура ответа:")
                        print(json.dumps(result.get("response_payload"), indent=2, ensure_ascii=False))

                audio_data = result.get("audio")
                if audio_data:
                    timestamp = int(time.time())
                    output_filename = f"alice_response_{timestamp}.opus"
                    save_audio_to_file(audio_data, output_filename)
                else:
                    print("[Алиса] < Аудиоответ не был получен (хотя запрашивался).")

            except YandexAliceClientError as e:
                print(f"Ошибка клиента Алисы при обработке запроса: {e}")
            ...
            except ValueError as ve:  # Например, пустой текст для TTS (хотя send_text это не вызовет)
                print(f"Ошибка ввода: {ve}")
            except Exception as e:
                import traceback
                print(f"Произошла непредвиденная ошибка при обработке запроса: {e}")
                traceback.print_exc()
        ...
        if client and client._is_connected: # Проверяем, что клиент был создан и подключен
            await client.close()
        else:
